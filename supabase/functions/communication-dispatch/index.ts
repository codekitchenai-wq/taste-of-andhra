// Provider-agnostic communication dispatcher (WhatsApp + SMS).
// Deploy: supabase functions deploy communication-dispatch
//
// Modes: single | drain | test
// Uses ProviderFactory — never calls Gupshup/Meta APIs directly.
...
Deno.serve(async (request) => {
  ...
  const requireAdmin = async (): Promise<Response | null> => {
    ...
    if (!membership) {
      return errorResponse('Only admins can dispatch communications.', 403)
    }
  ...
  if (mode === 'test') {
    ...
    const { data: settings } = await admin
      ...
      .select('whatsapp_provider, sms_provider')
    ...
    if (channel === 'whatsapp' && provider === 'meta_cloud') {
      const { data: config } = await admin
        .from('organization_whatsapp_configs')
        .select('template_map, access_token, phone_number_id')
        .eq('organization_id', organizationId)
        .maybeSingle()

      if (!config?.access_token || !config.phone_number_id) {
        return errorResponse(
          'WhatsApp is not connected. Save credentials first or use mock provider.',
          422,
        )
      }
    ...
    const idempotencyKey = `test:${channel}:${organizationId}:${Date.now()}`
