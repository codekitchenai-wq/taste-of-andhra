...
if (error?.context) {
  try {
    ...
    console.log('ERROR_CONTEXT_READ_FAIL', String(e))
  }
}

const connect = await client.functions.invoke('whatsapp-connect', {
  body: {
    organizationId: orgId,
    provider: 'meta_cloud',
    wabaId: '10728312611780785',
    phoneNumberId: '1243825012147715',
    displayPhoneNumber: '+15551997138',
    accessToken: 'mock',
  },
})
console.log('CONNECT_DATA', JSON.stringify(connect.data, null, 2))
console.log('CONNECT_ERROR', connect.error?.message)
if (connect.error?.context) {
  try {
    console.log('CONNECT_BODY', await connect.error.context.text())
  } catch {
    /* ignore */
  }
}
