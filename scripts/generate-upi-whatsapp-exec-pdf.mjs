/**
 * Generates docs/PAYMENT_UPI_WHATSAPP_EXEC_REVIEW.pdf
 * Usage: node scripts/generate-upi-whatsapp-exec-pdf.mjs
 */
...
const htmlPath = path.join(root, 'docs', 'PAYMENT_UPI_WHATSAPP_EXEC_REVIEW.html')
const pdfPath = path.join(root, 'docs', 'PAYMENT_UPI_WHATSAPP_EXEC_REVIEW.pdf')
...
try {
  ...
  if (!tryEdgePrint()) {
    console.error('Could not generate PDF. Install puppeteer or use Edge.')
    process.exit(1)
  }
}
